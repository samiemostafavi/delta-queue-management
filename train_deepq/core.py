...
def run_core(params, return_dict, queue, module_label: str) -> pl.DataFrame:
    ...
    # Train!
    logger.info(f"{params['run_number']}: training started")
    queue.learn(total_timesteps=params["until_train"])

    # write RL model to files
    # zip file
    queue.save(address=records_path + f"{params['run_number']}" + "_deepq_queue.zip")
    # json file
    dqjson = queue.json()
    with open(
        records_path + f"{params['run_number']}" + "_deepq_queue.json",
        "w",
        encoding="utf-8",
    ) as f:
        f.write(dqjson)

    logger.info(f"{params['run_number']}: training finished")

    # evaluation run
    logger.info(f"{params['run_number']}: starting evaluation")
    queue.rl_model_address = (
        records_path + f"{params['run_number']}" + "_deepq_queue.zip"
    )
    model.prepare_for_run(debug=False, clean=True)
    model.env.run(until=model.env.now + params["until_eval"])
    logger.info(f"{params['run_number']}: evaluation done")
...
def run_noaqm_core(params, return_dict, queue, module_label: str) -> pl.DataFrame:
